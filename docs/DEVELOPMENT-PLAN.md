...
| 4 | [Ingestion: PDF + OCR](./phases/phase-4-ingestion-pdf-ocr.md) | PDF & scanned-doc intake (learns like Excel) | ✅ | ✅ |
...
- **P3** — AI layer (`AiClient` + `AiEnricher` + `AiMappingIdentifier`): Stage-5 match-rescue
  on rule-failures → `ai_suggested` matches for confirmation, Stage-6 commentary; AI column
  mapping when auto-detect leaves gaps. Best-effort — a run never fails on AI error.
- **P4** — PDF ingestion (`PdfGridExtractor`): native (UglyToad.PdfPig) → vision
